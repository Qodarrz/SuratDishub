<x-app-layout>
	<x-slot name="header">
		<div class="flex items-center justify-between">
			<div>
				<h2 class="font-semibold text-xl text-gray-800 leading-tight">Tambah Surat Masuk</h2>
				<p class="text-sm text-gray-500">Isi data surat masuk dengan lengkap.</p>
			</div>
	...
	<div class="py-6">
		<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
			<div class="bg-white shadow-sm rounded-2xl border border-gray-100 p-6 space-y-6">
				...
				<form action="{{ route('surat-masuk.store') }}" method="POST" class="space-y-5">
					@csrf
				...
				<div class="space-y-1">
					...
					<select name="divisi" id="divisi" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm" required>
						...
						<option value="Daltib" @selected(old('divisi') === 'Daltib')>Daltib</option>
						<option value="Manrek" @selected(old('divisi') === 'Manrek')>Manrek</option>
						<option value="Parkir" @selected(old('divisi') === 'Parkir')>Parkir</option>
						<option value="TL" @selected(old('divisi') === 'TL')>TL</option>
					</select>
				</div>
