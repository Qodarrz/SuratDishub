	...
	<x-slot name="header">
		<div class="flex items-center justify-between">
			<div>
				...
				<p class="text-sm text-gray-500">Perbarui data surat keluar.</p>
			</div>
		...
		<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
			<div class="bg-white shadow-sm rounded-2xl border border-gray-100 p-6 space-y-6">
				...
				<form action="{{ route('surat-keluar.update', $suratKeluar) }}" method="POST" class="space-y-5">
					...
					<div class="space-y-1">
						<label for="surat_masuk_id" class="block text-sm font-medium text-gray-700">Terkait Surat Masuk (Cari & Pilih)</label>
						<input type="hidden" name="surat_masuk_id" id="surat_masuk_id_hidden" value="{{ old('surat_masuk_id', $suratKeluar->surat_masuk_id) }}" />
						<input type="text" id="surat_masuk_search" placeholder="Cari no surat atau perihal..." class="mt-1 block w-full rounded-lg border border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm px-3 py-2" />
						<div id="surat_masuk_results" class="mt-2 border border-gray-300 rounded-lg max-h-64 overflow-y-auto hidden">
							<!-- Hasil pencarian akan ditampilkan di sini -->
						</div>
						<p class="text-xs text-gray-400 mt-2">Pilih surat masuk yang akan ditandai selesai diproses (opsional).</p>
						<div id="surat_masuk_selected" class="mt-2 p-3 bg-blue-50 rounded-lg {{ $suratKeluar->surat_masuk_id ? '' : 'hidden' }}">
							<p class="text-sm text-blue-800"><strong>Dipilih:</strong> <span id="selected_text">{{ $suratKeluar->suratMasuk ? $suratKeluar->suratMasuk->no_surat . ' - ' . $suratKeluar->suratMasuk->perihal : '' }}</span></p>
						</div>
					</div>

					<div class="space-y-1">
						<label for="no_surat" class="block text-sm font-medium text-gray-700">No Surat</label>
						<input type="text" name="no_surat" id="no_surat" value="{{ old('no_surat', $suratKeluar->no_surat) }}" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm" required />
					</div>

					<div class="space-y-1">
						<label for="tanggal_surat" class="block text-sm font-medium text-gray-700">Tanggal Surat</label>
						<input type="date" name="tanggal_surat" id="tanggal_surat" value="{{ old('tanggal_surat', $suratKeluar->tanggal_surat) }}" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm" required />
					</div>

					<div class="space-y-1">
						<label for="perihal" class="block text-sm font-medium text-gray-700">Perihal / Deskripsi</label>
						<textarea name="perihal" id="perihal" rows="4" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm" required>{{ old('perihal', $suratKeluar->perihal) }}</textarea>
					</div>
