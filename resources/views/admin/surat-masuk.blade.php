<x-app-layout>
	<x-slot name="header">
		<div class="flex items-center justify-between">
			<div>
				<h2 class="font-semibold text-xl text-gray-800 leading-tight">Surat Masuk</h2>
				<p class="text-sm text-gray-500">Daftar seluruh surat masuk.</p>
			</div>
			<div class="flex items-center gap-2">
				<a href="{{ route('surat-masuk.create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg shadow hover:bg-indigo-700">
					<svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 4v16m8-8H4" />
					</svg>
					<span>Tambah Surat</span>
				</a>
				<form method="GET" action="{{ route('surat-masuk.index') }}" class="flex items-center">
					<div class="relative">
						...
						<input type="text" name="q" value="{{ $search ?? '' }}" placeholder="Cari no / tanggal / perihal" class="pl-9 pr-16 py-2 w-52 rounded-lg border border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500" />
				...
				<div class="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
					<div class="flex items-center">
						...
						<span class="text-sm text-blue-800">
							Menampilkan <strong>{{ $suratMasuk->total() }}</strong> hasil untuk pencarian "<strong>{{ $search }}</strong>"
						</span>
						<a href="{{ route('surat-masuk.index') }}" class="ml-auto text-xs text-blue-600 hover:text-blue-800">Clear</a>
					</div>
				</div>
			@endif
					...
					<table class="min-w-full divide-y divide-gray-200 text-sm">
						<thead class="bg-gray-50">
							<tr>
								...
								<th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Divisi</th>
						...
						<tbody class="bg-white divide-y divide-gray-100">
							@forelse($suratMasuk as $index => $item)
								...
								@else
									<tr>
								@endif
									<td class="px-4 py-3 text-gray-700">{{ $suratMasuk->firstItem() + $index }}</td>
									...
									<td class="px-4 py-3 text-right text-gray-700">
										<div class="inline-flex items-center gap-2">
											<a href="{{ route('surat-masuk.edit', $item) }}" class="inline-flex items-center justify-center h-8 w-8 rounded-full bg-indigo-50 text-indigo-600 hover:bg-indigo-100">
												<svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
											...
											<form action="{{ route('surat-masuk.destroy', $item) }}" method="POST" onsubmit="return confirm('Hapus surat ini?');">
												@csrf
												@method('DELETE')
							...
							@empty
								...
								@else
									<tr>
										<td colspan="6" class="px-4 py-6 text-center text-gray-500">Belum ada data surat masuk.</td>
									</tr>
								@endif
							@endforelse
						</tbody>
